...
export const usePlayer = (props?: Partial<UsePlayerProps>) => {
  ...
  const hasQueueProp = Array.isArray(queueProp) && queueProp.length > 0;

  const currentQueue = useMemo<Track[]>(() => {
    if (isShuffle && Array.isArray(shuffledQueue) && shuffledQueue.length > 0) {
      return shuffledQueue;
    }

    if (hasQueueProp) {
      return queueProp as Track[];
    }

    if (Array.isArray(reduxQueue) && reduxQueue.length > 0) {
      return reduxQueue;
    }

    return [];
  }, [queueProp, reduxQueue, shuffledQueue, isShuffle, hasQueueProp]);

  const stableQueue = hasQueueProp ? (queueProp as Track[]) : currentQueue;
  ...
  const handleShuffle = useCallback(
    async (startTrack?: Track, regenerateOnly = false) => {
      if (queueProp) {
        ...
        if (baseTrack) {
          const newQueue = generateShuffledQueue(baseTrack, reduxQueue ?? []);
